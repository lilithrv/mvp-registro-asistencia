...
describe("hashPassword", () => {
    it("CP-29: genera un hash distinto al texto plano", async () => {
...
describe("comparePassword", () => {
    it("CP-30: valida la contraseña correcta contra su hash", async () => {
        const hash = await hashPassword("Admin1234");
        expect(await comparePassword("Admin1234", hash)).toBe(true);
    });

    it("CP-31: rechaza una contraseña incorrecta contra su hash", async () => {
        const hash = await hashPassword("Admin1234");
        expect(await comparePassword("otraClave", hash)).toBe(false);
    });

    it("CP-32: valida la contraseña del administrador contra el hash del seed", async () => {
        expect(await comparePassword("Admin1234", SEED_HASH)).toBe(true);
    });

    it("CP-33: rechaza la contraseña del administrador escrita en minúsculas", async () => {
